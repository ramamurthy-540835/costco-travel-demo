...
| 4 | Booking & Rate Integrity (UC1, UC2, UC3) | 04-00, 04-01, 04-02, 04-03, 04-04, 04-05, 04-06, 04-07, 04-08, 04-09, 04-10, 04-11, 04-12, 04-13 | In progress — 04-00 through 04-06 and 04-09 through 04-12 done; 04-07/04-08 planned + adversarially reviewed, awaiting APPLY approval; 04-13 TBD (see entries below) | - |
...
- [ ] 04-07: Modification (closes UC2) — member changes dates on an existing booking through a real "Modify your booking" UI on the confirmation page AND a new real "My Bookings" account list page (dates-only in the UI; the API also supports inventory/vendor changes), backed by a new `lib/vendor-integration/policy.ts` boundary module's `quoteModification()` (wraps `searchInventory()`'s negotiated-rate lookup as the future Phase-6 RPC seam), re-applies the re-derived rate to a new `pricingSnapshot`, and requires a Stripe delta payment (via a live dry-run preview + Stripe Elements, mirroring 04-12's checkout pattern) for a price increase. **Depends on:** 04-05 (needs a real Booking to modify). **Files:** `lib/vendor-integration/policy.ts` (new), `lib/models/Booking.ts` (adds `modificationHistory`), `app/api/bookings/[id]/modify/route.ts`, `app/api/bookings/route.ts` (adds a `GET` listing handler + `buildOwnBookingsFilter()` helper), `components/booking-modify-form.tsx` (new), `app/(account)/bookings/page.tsx` (new — status/date-filtered list, per-row Modify), `components/header.tsx` (nav link), confirmation page. **Scope limit:** vendor-side re-confirmation remains a genuine Phase 6 concern; no automatic refund for a decreasing modification (04-08's territory); a captured-but-unapplied delta payment on quote drift is a disclosed, unhandled reconciliation gap. **PLANNING — 04-07-PLAN.md rewritten 2026-09-02 to add the UI + boundary module (2 rounds of adversarial review, 7 findings fixed) then extended same-day to add the My Bookings list page (1 further review round, 2 findings fixed), awaiting user review/APPLY approval.**
- [ ] 04-08: Cancellation & refund reconciliation (closes UC3) **+ automatic refund for decreasing modifications** — cancellation request confirmed through a real "Cancel booking" dialog on the confirmation page AND on the same "My Bookings" list page (refund preview shown before confirming), refund calculated via `lib/vendor-integration/policy.ts`'s `quoteCancellation()` from the real per-vendor cancellation policy (`VendorPolicy` graph node — including a new `no_show_fee_percent` field added by this plan), Stripe refund issued via `stripe.refunds.create` against the original PaymentIntent, guarded by an atomic status-claim against concurrent cancel requests. Also closes a gap surfaced live during 04-07's checkpoint: a modification that *decreases* the total previously left the member's original higher charge in place with no refund (04-07's UI disclosed this as "contact support"); Task 7 extends 04-07's modify route to automatically refund the difference via the same Stripe refund pattern, and updates its UI copy to confirm the refunded amount. **Depends on:** 04-05 (needs a real Booking + PaymentIntent to refund) **and 04-07** (Task 7 edits 04-07's own route/component — the one place these two plans have a real, one-directional dependency; everything else remains symmetric regardless of apply order). **Files:** `data/synthetic/vendor_policies.json`, `lib/graph/queries.ts` (widen `VendorPolicy` type), `lib/vendor-integration/policy.ts` (adds `quoteCancellation()` alongside 04-07's export), `app/api/bookings/[id]/cancel/route.ts`, `app/api/bookings/[id]/modify/route.ts` (04-07's route, Task 7 only), `components/booking-cancel-dialog.tsx` (new), `components/booking-modify-form.tsx` (04-07's component, Task 7 copy update), `lib/models/Booking.ts` (adds `cancellation` record + widens `modificationHistory` with `refundId`/`refundAmountCents`, removes dead `cancelRequest` field), `app/(account)/bookings/page.tsx` (per-row Cancel entry point, or creates the page if 04-07 hasn't landed yet), confirmation page. **Scope limit:** vendor-side cancellation notice is a Phase 6 concern; no two-step request/approve flow; a true no-show (cancelling after `from` has passed) uses the same rate as a standard inside-window cancellation, disclosed as a deliberate simplification (no seed field for a finer distinction). **PLANNING — 04-08-PLAN.md rewritten 2026-09-02 to add the UI + boundary module (2 rounds of adversarial review, 5 findings fixed), extended same-day to add the My Bookings cancel entry point (1 further review round, shared fixes with 04-07), then extended again 2026-09-03 to fold in the decreasing-modification refund (Task 7/AC-8, `depends_on` now includes 04-07) — awaiting user review/APPLY approval.**
...
- [ ] 04-13: TBD — Modify location/vehicle-type (extends UC2 beyond dates-only) — member changes the picked-up location and/or vehicle class/inventory on an existing `reserved` booking, not just dates. Distinct from 04-07's dates-only modification because it introduces two new concerns 04-07 deliberately does not handle: (1) an availability check against the target `inventoryId` (today nothing prevents "modifying" into inventory another member already holds — rate integrity, not inventory-locking, is this phase's stated scope), and (2) a full `searchInventory()`-style re-match against a potentially different vendor entirely (not just a re-quote of the same `inventoryId`/`vendorId` pair), including that vendor's own perk set, since perks are vendor-specific. `modificationHistory` (added by 04-07) already stores `inventoryId`/`vendorId` per entry, so no schema change is anticipated — this is additive to the modify route's matching logic, not a new audit model. **Depends on:** 04-07 (needs the modify route/UI/boundary module to extend) and, in effect, some inventory-availability signal that doesn't exist yet in the ontology (research needed — no vlabel currently tracks "held" vs. "available" inventory across concurrent bookings). Raised by the user 2026-09-03 during 04-07's live checkpoint testing; deliberately scoped as its own plan rather than folded into 04-07/04-08, which stay dates-only and cancellation-only respectively.
...
*Last updated: 2026-09-03 — added 04-13 (TBD): modify location/vehicle-type, extending UC2 beyond 04-07's dates-only scope; folded the decreasing-modification refund gap into 04-08 (new Task 7/AC-8, `depends_on` now includes 04-07, resequenced to wave 9), both raised by the user during 04-07's live checkpoint testing. 04-07/04-08 still awaiting user APPLY approval. See STATE.md's Accumulated Context for the full review findings.*
