...
| TD-9 | Fixed | Intermittent "no vehicles available" on `search_inventory` that succeeded on retry | Root cause: `lib/graph/client.ts`'s `pool.on('connect', ...)` callback ran AGE's session-scoped `LOAD 'age'; SET search_path...` fire-and-forget, racing a fresh connection's first query against its own still-pending setup — the retry "worked" only because it happened to reuse an already-warmed connection. Fixed by replacing the callback with an explicit `withGraphSession` helper that checks out a client, awaits setup, then runs the query on that same connection before releasing it. Verified with a 15-way concurrent `runCypher` smoke test — all succeeded, none raced. |
| TD-10 | Fixed | Wrong vehicle proposed: user asked to book a Mercedes-Benz shown in search results, `propose_booking` proposed a Toyota Corolla instead | Root cause: the model was passing back separate `inventoryId`/`vendorId` fields it could restate/mismatch independently, letting fields from two different result rows get paired. Fixed with an opaque composite `resultRef` (`inventoryId::vendorId`) token returned per search row and required by `propose_booking` instead of the two raw fields, plus a defense-in-depth `vehicleModel` belief-check against the resolved row. System prompt (`chat-loop.ts`) updated to match. Verified live end-to-end via Playwright: searched a model with 2 vendors, asked to book it (model correctly asked for vendor disambiguation instead of guessing), picked a vendor, confirmed the proposal named the exact requested vendor/model — no cross-row mixing. |
| TD-11 | Needs Phoenix tracing | Intermittent "no available rental cars" reported live (2026-09-15) for a real LA search with a "best rated vendors" narrowing + retry ("Check again") that both returned empty | Code-reviewed `search_inventory` (`tools.ts`), `searchInventory`/`runCypher`/`withGraphSession` (`lib/graph/*`) — no date-range filtering exists at search time (by design, availability is only gated later in `propose_booking`'s A2A `check_availability` call), no swallowed errors, TD-9's connection race fix is intact. "Best rated vendors" is not a tool parameter — the model ranks by `vendor.rating` itself over the raw result list, so an empty answer for that narrowing is plausibly an LLM ranking/reasoning decision, not a backend fault. Likely the same class of issue as TD-4 (LLM tool-call/reasoning inconsistency across turns) rather than a new code defect — needs a real trace of the failing turn's tool-call args to confirm. |
