...
def validate_trip_dates(pickup_at: str | datetime, drop_at: str | datetime, now: datetime | None = None) -> tuple[datetime, datetime]:
    ...
    minimum_date: date = current.date() + timedelta(days=1)
    if pickup.date() < minimum_date:
        raise ValueError("pickup_at must be tomorrow or later in the pickup timezone.")
    if drop <= pickup:
